...
var $author$project$Envelope$SliderChange = F2(
	function (a, b) {
		return {$: 'SliderChange', a: a, b: b};
	});
...
var $author$project$Envelope$init = function (str) {
	...
	var maxFormatter = function (value) {
		return '';
	};
	return {
		attack: A2(
...
var $author$project$ElmAndTone$initialModel = function () {
	...
	return {
		accordionState: $rundis$elm_bootstrap$Bootstrap$Accordion$initialState,
		...
		savedState: $elm$core$Maybe$Nothing,
		volumeSlider: A2(
			$carwow$elm_slider$SingleSlider$withMaxFormatter,
			maxFormatter,
			A2(
...
var $author$project$ElmAndTone$init = function (_v0) {
	return _Utils_Tuple2($author$project$ElmAndTone$initialModel, $elm$core$Platform$Cmd$none);
};
var $author$project$ElmAndTone$FXDropdownChange = function (a) {
	return {$: 'FXDropdownChange', a: a};
};
var $author$project$ElmAndTone$NoteOff = function (a) {
	return {$: 'NoteOff', a: a};
};
var $author$project$ElmAndTone$NoteOn = function (a) {
	return {$: 'NoteOn', a: a};
};
var $author$project$ElmAndTone$OSCDropdownChange = function (a) {
	return {$: 'OSCDropdownChange', a: a};
};
...
var $author$project$ElmAndTone$subscriptions = function (model) {
		...
		_List_fromArray(
			[
				...
				A2(
					...
					function (key) {
						return $author$project$ElmAndTone$NoteOn(key);
					},
				...
				A2(
					...
					$author$project$ElmAndTone$keyDecoder)),
				A2($rundis$elm_bootstrap$Bootstrap$Dropdown$subscriptions, model.oscillatorDropdown, $author$project$ElmAndTone$OSCDropdownChange),
...
var $author$project$Effect$buildSliders = F4(
	function (fxName, lst, values, settings) {
		var _v0 = _Utils_Tuple3(lst, values, settings);
		_v0$2:
		while (true) {
			if (!_v0.a.b) {
				if ((!_v0.b.b) && (!_v0.c.b)) {
				...
				if (_v0.b.b && _v0.c.b) {
					...
					var _v3 = _v0.c;
					var setting = _v3.a;
					var moreSettings = _v3.b;
					var valueFormatter = F2(
						function (value, not_used_value) {
							return hd + (': ' + $elm$core$String$fromFloat(value));
						});
					var minFormatter = function (value) {
						return '';
					};
					...
					return A2(
						$elm$core$List$cons,
						slider,
						A4($author$project$Effect$buildSliders, fxName, tail, rest, moreSettings));
				} else {
					break _v0$2;
				}
			}
...
var $author$project$Effect$init = F5(
	function (str, num, parameterString, parameterMinMax, parameterSettings) {
		return {
			effecting: str,
			...
			parameters: A4($author$project$Effect$buildSliders, str, parameterString, parameterMinMax, parameterSettings)
		};
	});
var $author$project$ElmAndTone$addEffect = function (str) {
	switch (str) {
		case 'Distortion':
			return _Utils_Tuple2(
				A5(
...
var $author$project$Effect$makeEffectString = F3(
...
var $author$project$ElmAndTone$mtof = function (midi) {
	return 440 * A2($elm$core$Basics$pow, 2, (midi - 69) / 12);
};
var $author$project$ElmAndTone$findKey = F2(
	function (s, m) {
		var test = function (n) {
			return _Utils_eq(n.key, s);
		};
		var mainVal = A2($elm$core$List$filter, test, m.notes);
		if (!mainVal.b) {
			return 0;
		} else {
			var hd = mainVal.a;
...
var $author$project$Effect$getChangedName = function (msg) {
	if (msg.$ === 'SliderChange') {
		var name = msg.a;
		return name;
	} else {
...
var $author$project$Effect$isRmMessage = function (ms) {
	if (ms.$ === 'RemoveEffect') {
		var str = ms.a;
		return true;
	} else {
...
var $author$project$ElmAndTone$updateAudio = _Platform_outgoingPort('updateAudio', $elm$json$Json$Encode$string);
var $author$project$ElmAndTone$makeAndSendAudio = function (lst) {
	return $author$project$ElmAndTone$updateAudio(
		A2(
			$elm$json$Json$Encode$encode,
			0,
			$elm$json$Json$Encode$string(lst)));
};
var $author$project$ElmAndTone$noteOff = F2(
	function (key, model) {
...
var $author$project$ElmAndTone$transposeUp = function (model) {
	return _Utils_update(
		model,
		{
			notes: A2(
...
var $author$project$Effect$changeParam = F4(
	function (name, val, paramNames, sliders) {
		var _v0 = _Utils_Tuple2(paramNames, sliders);
		_v0$2:
		while (true) {
			if (!_v0.a.b) {
				if (!_v0.b.b) {
...
var $author$project$Effect$update = F2(
	function (msg, env) {
		if (msg.$ === 'SliderChange') {
			...
			var typ = msg.b;
			var val = msg.c;
			var newList = A4($author$project$Effect$changeParam, typ, val, env.paramNames, env.parameters);
			...
			switch (typ) {
				...
				case 'release':
					return _Utils_update(
						env,
						{
							release: A2($carwow$elm_slider$SingleSlider$update, val, env.release)
						});
				default:
					return _Debug_todo(
						'Envelope',
...
var $author$project$ElmAndTone$update = F2(
	function (msg, model) {
		switch (msg.$) {
			...
			case 'NoteOn':
				var key = msg.a;
				var val = A2($author$project$ElmAndTone$findKey, key, model);
				var message = 'press-' + $elm$core$Debug$toString(val);
				return _Utils_Tuple2(
					A2($author$project$ElmAndTone$noteOn, key, model),
					$author$project$ElmAndTone$makeAndSendAudio(message));
			case 'NoteOff':
				var key = msg.a;
				var val = A2($author$project$ElmAndTone$findKey, key, model);
				var message = 'release-' + $elm$core$Debug$toString(val);
				return _Utils_Tuple2(
					A2($author$project$ElmAndTone$noteOff, key, model),
					$author$project$ElmAndTone$makeAndSendAudio(message));
			case 'TransposeUp':
				return _Utils_Tuple2(
					$author$project$ElmAndTone$transposeUp(model),
			...
			case 'EnvMessage':
				...
				var _v11 = A2($author$project$Envelope$update, envelopeMsg, model.addEnv);
				var newEnv = _v11.a;
				var str = _v11.b;
				return _Utils_Tuple2(
					_Utils_update(
			...
			case 'EffectMessage':
				...
				var name = A2(
					$elm$core$Debug$log,
					'Effect Name',
					$author$project$Effect$getChangedName(fXMsg));
				var comp = A2($elm$core$Dict$get, name, model.effects);
				if (comp.$ === 'Nothing') {
					return _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
				} else {
					var effect = comp.a;
					var rm = $author$project$Effect$isRmMessage(fXMsg);
					var _v13 = A2($author$project$Effect$update, fXMsg, effect);
					var fx = _v13.a;
					var message = _v13.b;
					return rm ? _Utils_Tuple2(
						_Utils_update(
							model,
							{
								effects: A2($elm$core$Dict$remove, name, model.effects)
							}),
						$author$project$ElmAndTone$makeAndSendAudio('removeFX-' + name)) : _Utils_Tuple2(
						_Utils_update(
							model,
							{
								effects: A3($elm$core$Dict$insert, name, fx, model.effects)
							}),
						$author$project$ElmAndTone$makeAndSendAudio('changeFX-' + message));
				}
			case 'OSCDropdownChange':
				var state = msg.a;
				return _Utils_Tuple2(
					_Utils_update(
						model,
					...
					$elm$core$Platform$Cmd$none);
			case 'FXDropdownChange':
				var state = msg.a;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{effectsDropdown: state}),
					$elm$core$Platform$Cmd$none);
			case 'AddFX':
				var effectName = msg.a;
				var _v14 = $author$project$ElmAndTone$addEffect(effectName);
				var newFX = _v14.a;
				var name = _v14.b;
				return _Utils_Tuple2(
					_Utils_update(
			...
			case 'OscillatorChange':
				var st = msg.a;
				var message = 'oscillator-' + st;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{oscillatorType: st}),
			...
			case 'PresetLoad':
				...
				if (str.$ === 'Nothing') {
					...
					var loadString = str.a;
					var nModel = A2($author$project$ElmAndTone$updateModel, str, model);
					return _Utils_Tuple2(
						nModel,
						$author$project$ElmAndTone$makeAndSendAudio(loadString));
				}
			case 'Save':
				if (!$elm$core$Basics$floor(
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								savedState: $elm$core$Maybe$Just(currState)
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					var currState = A2(
...
var $author$project$ElmAndTone$updateModel = F2(
	function (str, model) {
		if (str.$ === 'Nothing') {
			...
			var updateString = str.a;
			var sList = A2(
				$elm$core$Debug$log,
				'String',
				A2($elm$core$String$split, '#', updateString));
			...
			var foldfunc = F2(
				function (ms, ml) {
					var _v1 = A2($author$project$ElmAndTone$update, ms, ml);
					var newModel = _v1.a;
					var backms = _v1.b;
					return newModel;
				});
			var defModel = $author$project$ElmAndTone$initialModel;
			var x = A3($elm$core$List$foldl, foldfunc, defModel, lis);
			return x;
		}
	});
var $author$project$ElmAndTone$AccordionMsg = function (a) {
	return {$: 'AccordionMsg', a: a};
};
var $author$project$ElmAndTone$PresetLoad = function (a) {
	return {$: 'PresetLoad', a: a};
};
var $author$project$ElmAndTone$Save = {$: 'Save'};
var $author$project$ElmAndTone$TabChange = function (a) {
	return {$: 'TabChange', a: a};
};
var $author$project$ElmAndTone$TransposeDown = {$: 'TransposeDown'};
var $author$project$ElmAndTone$TransposeUp = {$: 'TransposeUp'};
var $author$project$ElmAndTone$UpdateContent = function (a) {
...
var $author$project$ElmAndTone$maybeStringToString = function (s) {
	if (s.$ === 'Nothing') {
...
var $author$project$ElmAndTone$getBlackOffset = F2(
	function (num, clr) {
		if (clr.$ === 'B') {
...
var $author$project$ElmAndTone$noteCSS = F3(
...
var $author$project$ElmAndTone$noteView = F2(
	function (i, note) {
		return A2(
			$elm$html$Html$div,
			_List_fromArray(
				[
					$elm$html$Html$Attributes$class(
					A3($author$project$ElmAndTone$noteCSS, i, note.triggered, note.clr)),
					$elm$html$Html$Attributes$class('Key'),
					A2($author$project$ElmAndTone$getBlackOffset, i, note.clr)
				]),
			_List_fromArray(
				[
...
var $author$project$Envelope$view = function (env) {
			...
			$rundis$elm_bootstrap$Bootstrap$Table$simpleThead(
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text('Attack/Decay')
							])),
						A2(
						$rundis$elm_bootstrap$Bootstrap$Table$th,
						_List_Nil,
						_List_fromArray(
							[
								$elm$html$Html$text('Sustain/Release')
							]))
					])),
			A2(
					...
					[
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$carwow$elm_slider$SingleSlider$view(env.attack)
									])),
								A2(
								$rundis$elm_bootstrap$Bootstrap$Table$td,
								_List_Nil,
								_List_fromArray(
									[
										$carwow$elm_slider$SingleSlider$view(env.sustain)
									]))
							])),
						A2(
						$rundis$elm_bootstrap$Bootstrap$Table$tr,
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$carwow$elm_slider$SingleSlider$view(env.decay)
									])),
								A2(
								$rundis$elm_bootstrap$Bootstrap$Table$td,
								_List_Nil,
								_List_fromArray(
									[
										$carwow$elm_slider$SingleSlider$view(env.release)
									]))
							]))
					]))));
};
var $author$project$Effect$RemoveEffect = function (a) {
	return {$: 'RemoveEffect', a: a};
};
...
var $rundis$elm_bootstrap$Bootstrap$Card$listGroup = F2(
	function (items, _v0) {
		var conf = _v0.a;
		return $rundis$elm_bootstrap$Bootstrap$Card$Config(
			_Utils_update(
				conf,
				{
					blocks: _Utils_ap(
						...
						_List_fromArray(
							[
								$rundis$elm_bootstrap$Bootstrap$Card$Internal$listGroup(items)
							]))
				}));
	});
...
var $author$project$Effect$view = function (env) {
	return $rundis$elm_bootstrap$Bootstrap$Card$view(
		A2(
			$rundis$elm_bootstrap$Bootstrap$Card$listGroup,
			_Utils_ap(
				A2($elm$core$List$map, $author$project$Effect$sliderView, env.parameters),
				_List_fromArray(
					[
						A2(
						$rundis$elm_bootstrap$Bootstrap$ListGroup$li,
									...
									[
										...
										_List_fromArray(
											[
												...
												$author$project$Effect$RemoveEffect(env.effecting))
											]))
									]),
								_List_fromArray(
									[
										$elm$html$Html$text('Remove' + env.effecting)
									]))
							]))
					])),
			A3(
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text(env.effecting)
							]))
					]),
...
var $author$project$ElmAndTone$viewEffect = F2(
	function (str, fx) {
		return A2(
			$elm$html$Html$div,
			_List_Nil,
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$carwow$elm_slider$SingleSlider$view(model.volumeSlider)
					])),
				A2(
				$elm$html$Html$pre,
				_List_Nil,
				...
				_List_fromArray(
					[
						$elm$html$Html$text('Type on the keyboard to play notes!')
					])),
				A2(
				$elm$html$Html$div,
				_List_Nil,
				A2($elm$core$List$indexedMap, $author$project$ElmAndTone$noteView, model.notes)),
				A2(
				$elm$html$Html$pre,
				_List_Nil,
				_List_fromArray(
					...
					[
							...
							[
								...
								_List_fromArray(
									[
										...
										$elm$html$Html$Events$onClick($author$project$ElmAndTone$TransposeUp)
									]))
							]),
						_List_fromArray(
							[
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$elm$html$Html$Events$onClick($author$project$ElmAndTone$TransposeDown)
									]))
							]),
						_List_fromArray(
							[
													...
													_List_fromArray(
														[
															...
															$author$project$ElmAndTone$PresetLoad(
																$elm$core$Maybe$Just('loadPreset-#gainenv+attack+0.0005#gainenv+decay+0.0005#gainenv+sustain+1#gainenv+release+0.7705#oscillator+sine#partial+0')))
														]))
												]),
											_List_fromArray(
												[
												...
												[
													...
													_List_fromArray(
														[
															...
															$elm$html$Html$Events$onClick(
															$author$project$ElmAndTone$PresetLoad(
																$elm$core$Maybe$Just('loadPreset-#gainenv+attack+0.0005#gainenv+decay+0.4905#gainenv+sustain+0.2405#gainenv+release+1.8705#oscillator+sine#partial+1')))
														]))
												]),
											_List_fromArray(
												[
												...
												[
													...
													_List_fromArray(
														[
															...
															$elm$html$Html$Events$onClick(
															$author$project$ElmAndTone$PresetLoad(
																$elm$core$Maybe$Just('loadPreset-#gainenv+attack+0.0005#gainenv+decay+0.5905#gainenv+sustain+0.1705#gainenv+release+0.0005#oscillator+square#partial+50')))
														]))
												]),
											_List_fromArray(
												[
													$elm$html$Html$text('Bright')
												])),
											A2(
											$rundis$elm_bootstrap$Bootstrap$Button$button,
											_List_fromArray(
												[
													...
													_List_fromArray(
														[
															...
															$elm$html$Html$Events$onClick(
															$author$project$ElmAndTone$PresetLoad(
																$elm$core$Maybe$Just('loadPreset-#gainenv+attack+0.0005#gainenv+decay+0.5905#gainenv+sustain+0.1705#gainenv+release+0.0005#oscillator+sawtooth#partial+50')))
														]))
												]),
											_List_fromArray(
												[
													$elm$html$Html$text('Plucky')
												])),
											A2(
											$rundis$elm_bootstrap$Bootstrap$Button$button,
											_List_fromArray(
								...
								pane: A2(
									...
									_List_fromArray(
										[
												...
												[
													...
													model.oscillatorDropdown,
													{
														items: _List_fromArray(
															[
																...
																_List_fromArray(
																	[
																		$elm$html$Html$Events$onClick(
																		$author$project$ElmAndTone$OscillatorChange('sine'))
																	]),
																_List_fromArray(
																	[
																...
																_List_fromArray(
																	[
																		$elm$html$Html$Events$onClick(
																		$author$project$ElmAndTone$OscillatorChange('square'))
																	]),
																_List_fromArray(
																	[
																...
																_List_fromArray(
																	[
																		$elm$html$Html$Events$onClick(
																		$author$project$ElmAndTone$OscillatorChange('triangle'))
																	]),
																_List_fromArray(
																	[
																		$elm$html$Html$text('Triange')
																	])),
																A2(
																$rundis$elm_bootstrap$Bootstrap$Dropdown$buttonItem,
																_List_fromArray(
																	[
																		$elm$html$Html$Events$onClick(
																		$author$project$ElmAndTone$OscillatorChange('sawtooth'))
																	]),
																_List_fromArray(
																	[
														...
														toggleButton: A2(
															...
															_List_fromArray(
																[
																	$elm$html$Html$text('Change Oscillator Type')
																])),
														toggleMsg: $author$project$ElmAndTone$OSCDropdownChange
													})
												])),
											A2(
											$elm$html$Html$div,
											...
											_List_fromArray(
												[
													$elm$html$Html$text('Toggle the sliders to create your own envelope:')
												])),
											A2(
											$elm$html$Html$div,
											_List_Nil,
											_List_fromArray(
												[
													...
													$elm$html$Html$map,
													$author$project$ElmAndTone$EnvMessage,
													$author$project$Envelope$view(model.addEnv))
												])),
											A2(
											$elm$html$Html$pre,
											_List_Nil,
											...
											$elm$core$Dict$values(
												A2($elm$core$Dict$map, $author$project$ElmAndTone$viewEffect, model.effects))),
											A2(
											$elm$html$Html$div,
											_List_Nil,
											_List_fromArray(
												[
													...
													model.effectsDropdown,
													{
														items: _List_fromArray(
															[
																...
																_List_fromArray(
																	[
																		$elm$html$Html$Events$onClick(
																		$author$project$ElmAndTone$AddFX('Distortion'))
																	]),
																_List_fromArray(
																	[
																...
																_List_fromArray(
																	[
																		$elm$html$Html$Events$onClick(
																		$author$project$ElmAndTone$AddFX('Chebyshev'))
																	]),
																_List_fromArray(
																	[
																		$elm$html$Html$text('Chebyshev')
																	])),
																A2(
																$rundis$elm_bootstrap$Bootstrap$Dropdown$buttonItem,
																_List_fromArray(
																	[
																		$elm$html$Html$Events$onClick(
																		$author$project$ElmAndTone$AddFX('FrequencyShifter'))
																	]),
																_List_fromArray(
																	[
														...
														toggleButton: A2(
															...
															_List_fromArray(
																[
																	$elm$html$Html$text('Add Effect')
																])),
														toggleMsg: $author$project$ElmAndTone$FXDropdownChange
													})
												]))
										]))
							})
						]),
					$rundis$elm_bootstrap$Bootstrap$Tab$config($author$project$ElmAndTone$TabChange))),
				A2(
				$elm$html$Html$pre,
				_List_Nil,
				_List_fromArray(
				...
				_List_fromArray(
					[
						$elm$html$Html$text('Save your current state and load it back in:')
					])),
				A2(
				$rundis$elm_bootstrap$Bootstrap$Button$button,
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$rundis$elm_bootstrap$Bootstrap$Utilities$Spacing$mr3,
								$elm$html$Html$Events$onClick($author$project$ElmAndTone$Save)
							]))
					]),
				_List_fromArray(
					[
					...
					[
						...
						_List_fromArray(
							[
								$rundis$elm_bootstrap$Bootstrap$Utilities$Spacing$mr3,
								$elm$html$Html$Events$onClick(
								$author$project$ElmAndTone$PresetLoad(model.savedState))
							]))
					]),
				_List_fromArray(
					[
				...
				A2(
							...
							$rundis$elm_bootstrap$Bootstrap$Accordion$card(
							{
											...
											[
												...
												_List_fromArray(
													[
														...
														_List_fromArray(
															[
																$elm$html$Html$text('Saved state: ')
															])),
														A2(
														$elm$html$Html$p,
														_List_Nil,
														_List_fromArray(
															[
																$elm$html$Html$text(
																$author$project$ElmAndTone$maybeStringToString(model.savedState))
															])),
														A2(
														$elm$html$Html$pre,
														_List_Nil,
														_List_fromArray(
															[
																$elm$html$Html$text('Manual load: ')
															])),
														A2(
														$elm$html$Html$input,
														_List_fromArray(
															[
																...
																$elm$html$Html$Attributes$placeholder('loadPreset-#'),
																$elm$html$Html$Attributes$value(model.formContent),
																$elm$html$Html$Events$onInput($author$project$ElmAndTone$UpdateContent)
															]),
														_List_Nil),
														A2(
														$rundis$elm_bootstrap$Bootstrap$Button$button,
															...
															[
																...
																_List_fromArray(
																	[
																		$elm$html$Html$Events$onClick(
																		$author$project$ElmAndTone$PresetLoad(
																			$elm$core$Maybe$Just(model.formContent)))
																	]))
															]),
														_List_fromArray(
															[
								...
								header: A2(
									...
									A2(
										...
										_List_fromArray(
											[
												$elm$html$Html$text('Manual Save/Load')
											]))),
								id: 'card1',
								options: _List_Nil
							})
					...
					$rundis$elm_bootstrap$Bootstrap$Accordion$withAnimation(
						$rundis$elm_bootstrap$Bootstrap$Accordion$config($author$project$ElmAndTone$AccordionMsg)))),
				A2(
				$elm$html$Html$pre,
				_List_Nil,
				_List_fromArray(
...
var $author$project$ElmAndTone$main = $elm$browser$Browser$element(
	{init: $author$project$ElmAndTone$init, subscriptions: $author$project$ElmAndTone$subscriptions, update: $author$project$ElmAndTone$update, view: $author$project$ElmAndTone$view});
_Platform_export({'ElmAndTone':{'init':$author$project$ElmAndTone$main(
