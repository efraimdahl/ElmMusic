...
buildSliders : String -> List String -> List ( Float, Float ) ->
  List ( Float, Float ) -> List (SingleSlider.SingleSlider Message)
  ...
  case ( lst, values, settings ) of
    ...
    ( hd :: tail, val :: rest, setting :: moreSettings ) ->
      let
        ...
        valueFormatter =
          \value not_used_value ->
            hd ++ ": " ++ String.fromFloat value
...
init : String -> Int -> List String -> List ( Float, Float ) ->
  List ( Float, Float ) -> Effect
...
changeParam : String -> Float -> List String ->
  List (SingleSlider.SingleSlider Message) ->
    List (SingleSlider.SingleSlider Message)
